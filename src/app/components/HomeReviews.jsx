      ...
      <div className="hidden xl:flex gap-[21px]">
        <div className="flex flex-col items-center justify-end mb-[56px] max-w-[342px] relative">
          ...
          <div className="flex gap-x-4 justify-start pt-4 md:pt-5 lg:pt-6 w-full">
            <button
              className={`cursor-pointer ${isBeginning ? "opacity-40 cursor-not-allowed" : "hover:opacity-80"}`}
              onClick={handlePrev}
            ...
            >
              ...
              <svg xmlns="http://www.w3.org/2000/svg" width="40" height="30" viewBox="0 0 40 30" fill="none">
                ...
                <defs><clipPath id="clip0_4603_124"><rect width="30" height="40" fill="white" transform="matrix(4.37114e-08 -1 -1 -4.37114e-08 40 30)" /></clipPath></defs>
              </svg>
            </button>

            <button
              className={`cursor-pointer ${isEnd ? "opacity-40 cursor-not-allowed" : "hover:opacity-80"}`}
              onClick={handleNext}
              disabled={isEnd}
            >
              {/* SVG Right Arrow */}
               <svg xmlns="http://www.w3.org/2000/svg" width="41" height="30" viewBox="0 0 41 30" fill="none">
                ...
                <defs><clipPath id="clip0_4603_121"><rect width="30" height="40" fill="white" transform="matrix(-4.37114e-08 1 1 4.37114e-08 0.423828 0)" /></clipPath></defs>
              </svg>
            </button>
          </div>
        </div>
          ...
          {reviews.map((review, index) => (
            <SwiperSlide key={index}>
              <div className="h-full transition-all duration-700  flex relative w-full max-h-[500px]">
                <p className={`text-[20px] font-bold absolute z-90 lg:w-[10rem] transition-all duration-700 leading-[24px]  ${activeIndex === index ? "text-[#1b1b1b] bottom-[180px] left-[30px]" : isEnd ? "right-0 bottom-[30px] text-center" : "text-white bottom-[30px] left-[30px]"}`}>
                  {review.title}
                </p>
                <div className={`relative z-0 w-5/12 bg-white h-[500px] p-6 content-end transition-all duration-500`}>
                  <div className="lg:max-w-[15rem] relative">
                    <p className={`text-[#1b1b1b] text-[14px] transition-all duration-500 ${activeIndex === index && showText ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
                      {review.text}
                    </p>
                  </div>
                </div>
                
                {/* Contenedor del Video/Imagen Desktop */}
                <div
                  className={`absolute top-0 h-[500px] transition-all duration-500 bg-cover bg-center right-0  ${activeIndex === index ? "w-7/12" : "w-full"}`}
        ...
        <div className="relative">
          <Swiper
            ...
            onSlideChange={(swiper) => {
                ...
                setActiveIndex(idx);
                setIsMuted(true); // Reset mute en móvil también
            }}
            className="mobile-reviews-swiper"
          >
            {reviews.map((review, index) => (
              <SwiperSlide key={index}>
                <div className="relative w-full overflow-hidden" style={{ minHeight: "655px" }}>
                  {activeIndex === index ? (
                    <>
                        <VideoControls youtubeLink={review.youtubeLink} />
                    ...
                    className={`absolute left-0 right-0 top-0 h-[455px] bg-cover bg-center transition-all duration-700 ease-out ${activeIndex === index ? "filter-none z-20" : "filter grayscale brightness-75 z-30"}`}
                    style={{ backgroundImage: `url(${review.img})` }}
                  />}

                  <div className={`absolute left-0 right-0 bottom-0 bg-white p-6 h-[200px] transition-transform duration-500 ease-out shadow-md ${activeIndex === index ? "translate-y-0 z-20 opacity-100 pointer-events-auto" : "-translate-y-full z-20 opacity-0 pointer-events-none"}`} aria-hidden={activeIndex !== index}>
