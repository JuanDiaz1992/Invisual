import HomeReviews from "../HomeReviews";
import ButtonLaid from "../button";
export default function CustomerSatisfaction() {
  return (
    <section
      id="satisfaction"
      className="min-h-screen text-white satisfaction lg:mt-[-120px] xl:pb-[200px] pt-[150px] mt-[-224px] lg:pt-0"
    >
      <div className="lg:container mx-auto lg:max-w-[1200px] px-[24px] xl:px-0 mt-[40px] lg:mt-0">
        <h2 className="text-[40px] leading-[40px] lg:text-[90px] font-black italic pb-8 lg:leading-[95px] ">
          CUSTOMER{" "}
          <span className="text-stroke block lg:text-[110px]">
            SATISFACTION
          </span>
        </h2>
        <p className="text-sm pointer-events-none select-none block max-w-[312px] md:max-w-[500px] lg:hidden leading-[22px] mb-[27px]">
          We do more than just car wrapping; we focus on understanding our
          clients needs, creating their branding and ensuring their absolute
          satisfaction. <br /> <br /> We invite you to see what some of our
          clients have to say about our service and to learn more about our
          work.
        </p>
        <div className="w-full flex lg:justify-end">
          <div className="flex flex-wrap content-end lg:pt-[67px] max-w-[370px] lg:max-w-full lg:w-4/12 mb-[53px] lg:mb-0">
            <p className="text-[22px] font-bold relative w-full lg:w-fit mb-[17px] lg:mb-0">Follow Us</p>
            <div className="flex flex-wrap gap-y-4 lg:pt-4 lg:gap-x-2">
              <ButtonLaid
                buttonText={"IN VISUAL SIGNS"}
                width={164}
                type="youtube"
              />
              <ButtonLaid
                buttonText={"@invisualsigns"}
                width={164}
                type="instagram"
              />
            </div>
          </div>
        </div>
      </div>
      <div className="lg:mt-[44px] lg:container mx-auto lg:max-w-[1200px] lg:pb-0">
        <HomeReviews />
      </div>
    </section>
  );
}
