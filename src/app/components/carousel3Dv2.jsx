...
const Carousel3Dv2 = ({ slides }) => {
  ...
  const imageVariants = {
    ...
    left1: { x: "-73%", scale: 0.8, zIndex: 3, rotateY: "-50deg" },
  ...
  return (
    <ul
      ...
      className="cursor-grab select-none active:cursor-grabbing flex items-center flex-col justify-center relative perspective-[1500px] min-h-[400px] mb-[-200px] md:mb-0"
    >
      {slides.map((slide, index) => {
        ...
        return (
          <motion.li
            key={index}
            className="rounded-[12px] bg-cover bg-center relative lg:w-[654px] lg:h-[350px] p-6 "
            style={{
              position: "absolute",
            }}
          ...
          >
            ...
            />
            <div
              className={`w-full h-full left-0 top-0 z-10 absolute transition-all ease-in-out ${
                isCenter ? "opacity-100" : "opacity-0"
              }`}
              style={{
                background:
                  "linear-gradient(177deg, rgba(0, 0, 0, 0.25) 20.93%, #1B1B1B 97.35%)",
              }}
            ></div>
            <div
              className={`z-10 relative mt-24 transition-all ease-in-out ${
                isCenter ? "opacity-100" : "opacity-0"
              }`}
            >
              <div className="lg:min-h-[160px]">
                <h3 className="text-2xl font-bold pointer-events-none select-none">
                  {slide.title}
                </h3>
                ...
                <Link
                  href={slide.link}
                  className="inline-block mt-4 font-bold underline cursor-pointer pointer-events-auto"
                  aria-label={`Learn more about ${slide.title}`}
                  tabIndex={isCenter ? 0 : -1}
                >
                  Learn More <span className="sr-only"> about {slide.title}</span>
                </Link>
            </div>
          </motion.li>
        );
